...
bool Unit::is_failed_lit(Lit l) {
  ...
  while (trail.size() > orig) {
    ...
    if (v < values.size()) values[v] = l_Undef;
    trail.pop_back();
  }
...
bool Unit::propagate() {
  while (!conflict && que_head < trail.size()) {
    if (!propagate(trail[que_head++])) conflict = true;
  }
  return !conflict;
}
...
void Unit::watch(Lit literal, size_t clause_index) {
  ...
  if (index >= watches.size()) watches.resize(index + 1, nullptr);
  if (watches[index] == nullptr) watches[index] = new vector<size_t>();
...
bool Unit::propagate(Lit literal) {
  ...
  if (value(variable) != l_Undef) return (literal_value == value(variable));
  ...
  // ws  are the clauses watched by literal, since the literal may stop watching
  // some clauses, the vector may shrink, we do that via two indices
  auto& ws = *(watches[li]);
  ...
  while (read_index < ws.size()) {
    ...
    if (new_watch_index > 0) {  // new watch found
      // swap the new watch into index 1, and the old watch literal into its
      // index
  ...
  if (write_index < ws.size()) ws.resize(write_index);
  return return_value;
}
